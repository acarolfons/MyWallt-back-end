...
export async function getTransaction(req, res) {
  ...
  try {
    ...
    if (page <= 0 || isNaN(page))
      return res
        .status(400)
        .send("Página inválida. Deve ser um número positivo.");
    ...
    const formattedTransactions = transactions.map((transaction) => {
      return {
        ...transaction,
        createdAt: dayjs(transaction.createdAt).format("DD/MM"), // Formata para DD/MM
      };
    });
...
export async function updateTransaction(req, res) {
  ...
  try {
    ...
    if (!transaction.userId.equals(userId))
      return res.status(401).send("Você não pode editar esta transação");
...
export async function deleteTransaction(req, res) {
  ...
  try {
    ...
    if (!transaction.userId.equals(userId))
      return res.status(401).send("Você não pode deletar esta transação");
